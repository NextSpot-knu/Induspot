...
def _build_prompt(utterance: str, facility_type_ko: str, current_name: Optional[str], candidates: list[dict]) -> str:
    ...
    return (
        ...
        "'국밥'→'돼지국밥 국밥 해장국 수육백반', '국수'→'칼국수 잔치국수 멸치국수 국수', '순대'→'순대 순댓국 순대국밥', "
        "'찌개'→'김치찌개 된장찌개 부대찌개 전골'. "
        "요리 이름 위주로 확장하고(재료만 나열 금지), 서로 다른 인접 분류는 섞지 마세요: "
        "고깃집(삼겹살·갈비) / 곱창집 / 순댓국(순대) / 국밥집(돼지국밥·해장국) / 보쌈집 / 치킨(후라이드·양념) / "
        "닭갈비·찜닭 / 초밥·일식 / 해물·생선 / 국수·칼국수 / 중식(짜장·짬뽕) / 양식(피자·파스타) 은 각각 별개입니다. "
        "특히 '국밥'은 순댓국이 아니라 돼지국밥·해장국 쪽으로, '피자'는 양식 쪽으로 확장하세요. "
