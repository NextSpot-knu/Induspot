"""자연어 선호 입력 → 추천 알고리즘 반영 엔드포인트 (Gemini 활용).

POST /api/v1/preferences/parse
  body: { "text": "조용한 회의실이랑 전기차 충전되는 주차장이 좋아요" }

흐름:
  1) Gemini(preference_nlp_service)로 자연어를 구조화(선호 카테고리/속성/요약/8차원 벡터)
  2) 그 8차원 벡터를 Pinecone 사용자 선호 벡터로 upsert
...
class ParsePreferenceRequest(BaseModel):
    text: str = Field(..., description="근로자가 자연어로 말한 선호(예: '조용한 회의실 선호')")
